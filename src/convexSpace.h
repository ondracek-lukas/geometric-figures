...
struct convexSpace {
	...
	GLfloat *pos;      // a point in the space, it should be inside the figure when assigned
	GLfloat *ortBasis; // orthonormal basis
	GLfloat hash;      // distance to a fixed random point, introduces order of spaces
	GLfloat *normal, normalPos; // normal (in another space) and position within it (dist from origin)
};
...
extern void convexSpaceCreateVert(struct convexSpace **pSpace, GLfloat *pos);
...
extern GLfloat convexSpaceDist(struct convexSpace *space, struct convexSpace *vert);

// Returns squared distance of space and given vertex
extern GLfloat convexSpaceDistSq(struct convexSpace *space, struct convexSpace *vert);

// Returns oriented distance of space and given vertex (in direction of space->normal)
extern GLfloat convexSpaceOrientedDist(struct convexSpace *space, struct convexSpace *vert);
